...
@property (nonatomic, strong) MMLocationBlock mmLocationBlock;
...
- (void) getLocationCoordinateWithMMLocationBlock:(MMLocationBlock) mmLocaiontBlock {
   
    self.mmLocationBlock = [mmLocaiontBlock copy];
    [self startLocation];
}
...
- (void)mapView:(MKMapView *)mapView didUpdateUserLocation:(MKUserLocation *)userLocation
{
    ...
    CLGeocodeCompletionHandler handle = ^(NSArray *placemarks,NSError *error)
    {
        for (CLPlacemark * placeMark in placemarks)
        {
            ...
            NSString *lastTime = [NSString stringWithFormat:@"%@", [formatter stringFromDate:[NSDate date]]];
            
            [standard setObject:self.lastCity forKey:MMLastCity];
            [standard setObject:self.lastAddress forKey:MMLastAddress];
            [standard setObject:lastTime forKey:MMLastTime];
            
        }
        
        if (_mmLocationBlock){
            _mmLocationBlock(_lastCoordinate,_lastAddress);
            _mmLocationBlock = nil;
        }
